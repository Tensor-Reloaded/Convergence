...
DatasetType = Union[torchvision.datasets.CIFAR10,
                    torchvision.datasets.CIFAR100, torchvision.datasets.MNIST]
...
def main(config: DictConfig):
    ...
    STORAGE_DIR = os.path.join(os.path.dirname(
        utils.get_original_cwd()), "storage")
    SUBSET_INDICES_DIR = os.path.join(STORAGE_DIR, 'subset_indices')
    OUTPUT_BATCHES_PERMUTATIONS_DIR = os.path.join(
        STORAGE_DIR, 'output_batches_permutations')
...
class Solver(object):
        ...
        if self.args.train_subset == None and self.args.classes_subset == None:
            if self.args.orderer == "baseline":
                ...
                if self.args.orderer == "multi_attempt":
                    orderer = MultiAttemptOrderer(
                        dataset=train_set, model=self.model, batch_size=self.args.train_batch_size, criterion=self.criterion, nr_attempts=self.args.nr_attempts)
                    ...
                    if not self.args.static_batch_size == None:
                        ...
                        del aux_loader
                    orderer = MaxLossDeltaOrderer(dataset=train_set, model=self.model, optimizer=self.optimizer, batch_size=self.args.train_batch_size, criterion=self.criterion,
                                                  nr_attempts=self.args.nr_attempts, device=self.device, delta_loss_type="absolute", static_batch_target=static_batch_target)
                elif self.args.orderer == "batch_loss_shuffler":
                    print("This orderer is not implemented, go ahead an commit one")
                    exit()

                self.train_loader = torch.utils.data.DataLoader(
                    dataset=train_set, batch_sampler=orderer)
        else:
    ...
    def _build_subset_loader(
        ...
        filename = os.path.join(
            SUBSET_INDICES_DIR,
            "subset_balanced_{}_{}_{}_{}.data".format(
                self.args.dataset, train_or_test, classes, n_samples)
        )
    ...
    def set_device(self):
        if self.cuda:
            self.device = torch.device(
                'cuda' + ":" + str(self.args.cuda_device))
        ...
        if self.args.scheduler == "ReduceLROnPlateau":
            ...
            if self.args.sum_augmentation:
                self.scheduler = optim.lr_scheduler.CosineAnnealingLR(
                    self.optimizer, T_max=self.args.epoch//(self.args.nr_cycle-1), eta_min=self.args.reduce_lr_min_lr)
            else:
                self.scheduler = optim.lr_scheduler.CosineAnnealingLR(
                    self.optimizer, T_max=self.args.epoch, eta_min=self.args.reduce_lr_min_lr)
            ...
            self.scheduler = optim.lr_scheduler.OneCycleLR(self.optimizer, max_lr=self.args.lr, total_steps=None, epochs=self.args.epoch//(self.args.nr_cycle-1), steps_per_epoch=len(
                self.train_loader), pct_start=0.3, anneal_strategy='cos', cycle_momentum=True, base_momentum=0.85, max_momentum=0.95, div_factor=10.0, final_div_factor=500.0, last_epoch=-1)
        else:
    ...
    def train(self):
        ...
        for batch_num, (data, target) in enumerate(self.train_loader):
            ...
            self.writer.add_scalar(
                "Train/Batch_Loss", loss.item(), self.get_train_batch_plot_idx())
    ...
    def test(self):
        ...
        with torch.no_grad():
            for batch_num, (data, target) in enumerate(self.test_loader):
                ...
                loss = self.criterion(output, target)
                self.writer.add_scalar(
                    "Test/Batch_Loss", loss.item(), self.get_test_batch_plot_idx())
                total_loss += loss.item()
                prediction = torch.max(output, 1)
    ...
    def save(self, epoch, accuracy, tag=None):
        ...
        model_out_path = os.path.join(
            self.save_dir, "model_{}_{}{}.pth".format(epoch, accuracy * 100, tag))
    ...
    def run(self):
        if self.args.all_batch_permutations:
            ...
            if self.args.seed is not None:
                ...
                print(
                    f'Initialized before loading model and data with seed {self.args.seed}')
            self.load_model()
            self.load_data()
    ...
    def _get_or_generate_train_permutation(self, epoch, size):
        f = os.path.join(OUTPUT_BATCHES_PERMUTATIONS_DIR,
                         f'train_indices_permutation_epoch={epoch}_size={size}.pt')
        if os.path.isfile(f):
            return torch.load(f)
    ...
    def _get_or_generate_train_indices(self):
        if self.args.train_indices_file:
            ...
            out_f = os.path.join(
                OUTPUT_BATCHES_PERMUTATIONS_DIR, 'train_indices.pt')
    ...
    def run_all_batch_permutations(self):
        ...
        test_indices = self._build_subset_indices(
            self.test_set, n_samples=None, classes=self.args.classes_subset)
        ...
        if self.args.load_model:
            base_model_name = self.args.load_model[self.args.load_model.find(
                self.args.model):self.args.load_model.find(')') + 1]
            ...
            self.load_model()
            out_f = os.path.join(
                OUTPUT_BATCHES_PERMUTATIONS_DIR, base_model_name + '.pt')
        ...
        for epoch in range(FIRST_EPOCH, LAST_EPOCH + 1):
            self._run_all_batch_permutations_epoch(
                base_model_name, base_model_state_dict, epoch)

    def _run_all_batch_permutations_epoch(self, base_model_name, base_model_state_dict, epoch):
        if epoch == 0:
            prepare_epochs_batches_list = [[]]
        else:
            prepare_epochs_batches_list = self._get_prepare_epochs_batches_list(
                base_model_name, epoch - 1)

        train_indices_perm = self.train_indices[self._get_or_generate_train_permutation(
            epoch, self.train_indices.size(0))]
        ...
        with open(out_csv, 'w', newline='') as f:
            ...
            csv_header.extend(
                ['FinalBatches', 'FinalBatchesIndexes', 'FinalPermutationIndex'])
            csv_header.extend(
                ['TestLoss', 'TestCorrect', 'TestTotal', 'TestAcc'])
    ...
    def _get_prepare_epochs_batches_list(self, model: str, epoch: int) -> List[List[List[int]]]:
        ...
        N_CLUSTERS = 12
        clusters = KMeans(n_clusters=N_CLUSTERS).fit_predict(
            losses.reshape(-1, 1))
        idx_for_each_cluster = [
            np.where(clusters == i)[0][0] for i in range(N_CLUSTERS)]
    ...
    def _do_all_final_epoch_permutations(self, base_model_state_dict, details: str, prepare_epochs_batches, final_epoch_batches, csv_writer):
        ...
        batches_idxs_perm = itertools.permutations(
            range(len(final_epoch_batches)))

        start_time = time.time()

        for i, (final_batches, final_batches_idxs) in enumerate(zip(batches_perm, batches_idxs_perm)):
            self._do_prepare_before_all_batch_perms(
                base_model_state_dict, prepare_epochs_batches)
            ...
            csv_row.extend([i, test_loss, test_correct,
                            test_total, test_correct / test_total])
            csv_writer.writerow(csv_row)

            if (i + 1) % BATCH_PERMUTATIONS_LOG_FREQUENCY == 0:
                print(
                    f'Done {details}, perms={i + 1} in {time.time() - start_time:.2f} sec')
    ...
    def _do_run(self):
        ...
        try:
            for epoch in range(1, self.args.epoch + 1):
                ...
                self.writer.add_scalar(
                    "Model/Norm", self.get_model_norm(), epoch)
                self.writer.add_scalar(
                    "Train_Params/Learning_rate", self.optimizer.param_groups[0]['lr'], epoch)
        ...
        paths = [os.path.join(self.save_dir, basename)
                 for basename in files if "_0" not in basename]
        if len(paths) > 0:
            src = max(paths, key=os.path.getctime)
            copyfile(src, os.path.join(
                "runs", self.args.save_dir, os.path.basename(src)))
